...
static inline uint64_t ceil_div(uint64_t num, uint8_t div) {
    return (num-1) / div + 1;
}

// es. 9 bit requires 2 byte -> byte_required(9) -> 2
// this function return at least 1
static inline uint64_t bytes_required(uint64_t bits) {
...
static inline void clear_bit(uint8_t *byte, uint8_t i) {
    *byte &= ~(1 << (7-i));
}
...
static inline uint8_t take_few_bits(uint8_t byte, uint8_t bit_length) {
    assert(bit_length < 8);
    return byte >> (8 - bit_length);
}

static inline void assign_bit(uint8_t *v, uint64_t bit_index, bool value) {
...
static inline bool access_bit(const uint8_t *v, uint64_t bit_index) {
    const uint64_t byte_idx = bit_index >> 3; // (i/8)
    ...
    static inline uint64_t calc_align_index_based(uint64_t index, uint64_t align) {
    ...
    // return bytes
    static inline uint64_t calc_mem_size(uint64_t new_bit_capacity) {
